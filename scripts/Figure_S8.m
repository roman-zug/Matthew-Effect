% Script: Figure_S8
%
% Purpose: This script plots Supplementary Figure S8, which shows the 
...
% (columns), with age dependence, which means that production of sexual eggs 
% does not start above sexual threshold y (there is no such threshold in 
% the model version with age dependence), but rather after a given time point 
% called 't_sexual'.
...
[xopt, max_value, iopt] = find_optimum_with_age_dependence(TSmean, x);
...
% Adjust the call to matthew_with_age_dependence to match the initial call structure
params1 = ones(size(x1(:))) * [a1 a2 a3 mu_q0 mu_w];

[L1, TE1, TS1, RE1, RS1] = matthew_with_age_dependence(params1, x1(:), t_sexual);
...
[L2, TE2, TS2, RE2, RS2] = matthew_with_age_dependence(params2, x2(:), t_sexual);
